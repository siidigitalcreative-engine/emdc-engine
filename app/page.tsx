              ...
              {sourceRows.map((item:any)=>{
                ...
                const generateDetailedProductIntroImagePrompt = () => {
                  const productList = tableProducts.map((row:any,idx:number)=>{
                    ...
                    return [
                      `${idx+1}. ${row.product || "Product"}${row.sku ? ` (${row.sku})` : ""}`,
                      row.brand ? `Brand: ${row.brand}` : "",
                      row.category ? `Category: ${row.category}` : "",
                      rowLinks.length ? `Product image reference link/s: ${rowLinks.join(" | ")}` : "Product image reference link/s: none found",
                    ].filter(Boolean).join("\n");
                  }).join("\n\n");

                  const visualGoal = tableProducts.length > 1
                    ? "Create one cohesive premium ecommerce hero image featuring the selected product collection together. Arrange the products clearly so every SKU is visible and recognizable. Use a clean balanced composition with the hero products centered, premium spacing, soft natural shadows, and a polished marketplace-ready look."
                    : "Create one premium ecommerce hero image focused on the selected product. Keep the product centered, clear, accurately proportioned, and marketplace-ready with soft natural shadows and a clean composition.";
                  ...
                  const generatedPrompt = [
                    ...
                    "",
                    "FEATURED PRODUCTS AND REFERENCE LINKS:",
                    productList || "No product details found.",
                    "",
                    "IMAGE OBJECTIVE:",
                    visualGoal,
                    "",
                    ...
                    tableProducts.length > 1
                      ? "Arrange the collection in a visually pleasing group shot with the main SKU slightly forward and supporting SKUs around it. Maintain correct size relationships between products. Leave enough breathing room around the products for future layout use."
                      : "Use a centered hero-product composition with enough clean space around the product for future layout use.",
                    "",
                    "QUALITY STYLE:",
                    "Hyper-realistic, premium commercial product photography, sharp focus, natural colors, accurate materials, clean modern ecommerce styling, no text overlay.",
                  ].filter(Boolean).join("\n");

                  updateProductIntroDigitalItem(item.id,{ imagePrompt:generatedPrompt });
                  markActionDone(`detailed-prompt-${item.id}`);
                };
...
const AIAdTemplates = ({ skuStorage=[], brands=[], hideProductSelector=false, presetProducts=[], onSendToDC=null, onAddToOverview=null, mainPromotion="", generatedOutputsStorageKey="global" }: any) => {
  ...
  const getCurrentGeneratedAdOutput = () => isCarouselFormat ? formatCarouselCardsForCopy(generatedAdCards) : generatedAdText;

  const sendGeneratedAdToDC = () => {
    const output = getCurrentGeneratedAdOutput();
    if(!String(output || "").trim()){
      setAdError("Generate an ad first before sending to Digital Creative.");
      return;
    }
    if(onSendToDC) {
      onSendToDC({
  ...
  const sendSavedAdToDC = (item:any) => {
    if(!item || !onSendToDC) return;
    onSendToDC({
      text:item.text || "",
      ...
      products:Array.isArray(item.products) && item.products.length ? item.products : effectiveAdSkus,
      brief:item.brief || "",
  ...
  const sendGeneratedBatchOutputToDC = (item:any) => {
    ...
    onSendToDC({
      ...
      formatName:item.formatName || "",
      templateName:item.templateName || "",
      products:Array.isArray(item.products) ? item.products : effectiveAdSkus,
      brief:item.brief || adBrief,
      isCarousel:!!item.isCarousel,
      isCollection:!!item.isCollection,
  ...
  return (
    <div style={{ background:C.surface,border:`1.5px solid ${C.border}`,borderRadius:12,padding:isMobile?14:20,maxWidth:"100%",overflow:"hidden" }}>
                  ...
                  {generatedBatchOutputs.map((item:any)=>(
                    <div key={item.id} style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden" }}>
                      <div style={{ padding:"10px 12px",borderBottom:`1px solid ${C.border}`,background:C.surfaceAlt,display:"flex",justifyContent:"space-between",gap:8,alignItems:"flex-start" }}>
                        <div style={{ minWidth:0 }}>
                          <p style={{ margin:0,fontSize:12,fontWeight:900,color:C.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{item.platformName || "Platform"} · {item.formatName || "Ad Format"}</p>
                          <p style={{ margin:"3px 0 0",fontSize:10.5,color:C.muted }}>{Array.isArray(item.products)?item.products.length:0} product{Array.isArray(item.products)&&item.products.length!==1?"s":""} selected for this ad</p>
                        </div>
                      </div>
                      <div style={{ padding:12,display:"flex",flexDirection:"column",gap:10 }}>
                        <div style={{ padding:"8px 10px",border:`1px solid ${C.border}`,borderRadius:9,background:C.bg,maxHeight:90,overflowY:"auto" }}>
                          <p style={{ margin:"0 0 4px",fontSize:10.5,fontWeight:900,color:C.textSub,textTransform:"uppercase",letterSpacing:".05em" }}>Products</p>
                          {(Array.isArray(item.products)?item.products:[]).map((product:any,idx:number)=>(
              ...
              {savedAdTemplates.length===0 ? (
                ...
                <div style={{ display:"flex",flexDirection:"column",gap:8,maxHeight:220,overflowY:"auto" }}>
                  {savedAdTemplates.map((item:any)=>(
                    <div key={item.id} onClick={()=>setSavedAdPreview(item)} title="Click to open saved output" style={{ padding:10,borderRadius:9,background:C.bg,border:`1px solid ${C.border}`,cursor:"pointer",display:"flex",gap:10,alignItems:"flex-start" }}>
                      <div style={{ minWidth:0,flex:1 }}>
      ...
      <Modal open={!!savedAdPreview} onClose={()=>setSavedAdPreview(null)} title="Saved Ad Output" width={760}>
        {savedAdPreview&&(
          <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
            <div style={{ padding:12,background:C.surfaceAlt,border:`1px solid ${C.border}`,borderRadius:10 }}>
              <p style={{ margin:"0 0 3px",fontSize:13,fontWeight:900,color:C.text }}>{savedAdPreview.platformName || "Platform"} · {savedAdPreview.formatName || "Ad Format"}</p>
              <p style={{ margin:0,fontSize:11,color:C.muted }}>{savedAdPreview.templateName || "Saved Output"} · {savedAdPreview.createdAt ? new Date(savedAdPreview.createdAt).toLocaleString("en-PH") : "Saved"}</p>
            </div>
            <div style={{ maxHeight:420,overflowY:"auto",WebkitOverflowScrolling:"touch",padding:14,background:C.bg,border:`1px solid ${C.border}`,borderRadius:10 }}>
              <pre style={{ margin:0,whiteSpace:"pre-wrap",wordBreak:"break-word",fontFamily:"inherit",fontSize:12.5,lineHeight:1.55,color:C.text }}>{savedAdPreview.text || ""}</pre>
            </div>
            <div style={{ display:"flex",justifyContent:"flex-end",gap:8,flexWrap:"wrap" }}>
