...
const DATA_PATH = "emdc-state/checklist-items/all.json";
...
export async function GET() {
  const checklistItems = await readJson(DATA_PATH, {});
  return NextResponse.json(
    { ok: true, checklistItems },
...
export async function POST(req: NextRequest) {
  try {
    ...
    await put(DATA_PATH, JSON.stringify(checklistItems), {
      access: "private", addRandomSuffix: false, allowOverwrite: true,
      contentType: "application/json",
    } as any);
    ...
    return NextResponse.json({
      ...
      checklistItemsVersion: sync.checklistItemsVersion,
    });
  } catch (error: any) {
    return NextResponse.json(
      { ok: false, error: error?.message || "Unable to save checklist items." },
      { status: 500 }
    );
  }
}
